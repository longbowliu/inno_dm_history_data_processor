        ...
        if response.status_code == 200:
            ...
            try:
                with connection.cursor() as cursor:
                    innopc_ids = []
                    if uuid1:
                        sql = "SELECT id FROM dm_innopc WHERE attach_id = %s"
...
def upload_metadata_to_scene(innopc_ids, scene_id,dataset_dir,dataset_name,group_name):
    ...
    if not scene_id:
        ...
        return
    meta_group_data = {
        "name": f"{dataset_name}_MetaGroup_{group_name}",
        "metaIds": [],
        "metas": []
    }
    ...
    if flatten_yaml:
        ...
        print("[⚠️] 未找到 Flatten YAML")
          ...
          # 添加 flatten_yaml_id 和 fusion_yaml_id 到 metaIds
    if flatten_yaml_id:
        flatten_yaml_json =  {
    ...
    url = f"http://localhost/dmapi/scene/{scene_id}/metadata"
    headers = {
        "Content-Type": "application/json"
    }
